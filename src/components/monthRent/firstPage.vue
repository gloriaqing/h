...
<template>
	<div v-show="hide">
		...
		<div style="background: #f0f0f0;
			...
			<div style="width: 30%;float: left;margin-left: 11px;" @click="goMoment">
				<img src="../../assets/img/icon/临时点名 (3).png" style="width:65px;margin-top: -10px;" />
				<p style="font-size:18px;color: #1095D9;margin-top: 5px;">临停缴费</p>
			</div>
			<div style="width: 30%;float: left;margin-left: 8px;" @click="goMonth">
				<img src="../../assets/img/icon/月报 (2).png" width="50px" />
				<p style="font-size:18px;color: #1095D9;margin-top: 10px;">长租续费</p>
			</div>
			<div style="width: 30%;float: left;" @click="goLock">
		...
		<div style="position: absolute;top:63%;width:100%;
			background: #f0f0f0;">
			<div style="width: 30%;float: left;margin-left: 11px;" @click="goInfo">
			...
			<div style="width: 30%;float: left;margin-left: 8px;" @click="goCount">
				<img src="../../assets/img/icon/账单查询.png" style="margin-top: 3px;" width="50px" />
				<p style="font-size:18px;color: #1095D9;margin-top: 10px;">账单查询</p>
			</div>
			<div style="width: 30%;float: left;" @click="goPon">
				<img src="../../assets/img/icon/优惠卡券.png" width="50px" />
				<p style="font-size:18px;color: #1095D9;margin-top: 11px;">优惠卡券</p>
			</div>
		</div>
...
<script>
	...
	export default{
		...
		methods:{
			...
			goMoment(){this.$router.push({path:'/selectCar'})},
			goMonth(){this.$router.push({path:'/repay'})},
			goLock(){
				//this.$router.push({path:'/lockCar'})
				window.location.href='http://www.yangchenkeji.com.cn/userpay/lockCar'
				},
			goCount(){
				sessionStorage.removeItem('knowLastTime')
				this.$router.push({path:'/countDetail'});
	    ...
	    this.$http.get("http://prod20.yc-yunpass.com/cloud/api/user/bind/findUserId?openId=" + ds+"&type="+type
	      ).then(res=> {
	      	console.log("res的数据",res);
	      	console.log("res的data的数据",res.data);
	      	if(res.data.errorMsg == '该用户未进行绑定' ||res.data.data.userId == 'null' || res.data.data.userId == null){
	      		this.$router.push({path:'/bindPhone?openid='+ds+'&type='+type});
	      		//window.location.href="http://prod20.yc-yunpass.com/userpay/bindPhone?openid="+ds+"&type="+type;
	      	}else{
	      		this.hide=true
